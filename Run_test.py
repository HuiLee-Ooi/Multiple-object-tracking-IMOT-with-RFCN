  ...
  with tf.Session(graph=detection_graph) as sess:
    ...
    # while (cap.isOpened()):
      ...
      for i in range((np.squeeze(num_detections))):
        ...
        if (scores is None or final_score[i] > 0.5) and ((np.reshape(classes,int(np.squeeze(num_detections)),1)[i]) !=7.0):
            count = count + 1
            # print (np.reshape(classes,int(np.squeeze(num_detections)),1)[i])
            # print (final_score[i])
            # class_name = category_index[np.reshape(classes,int(np.squeeze(num_detections)),1)[i]]['name']
            # print (class_name)
            ...
            # print ([np.reshape(classes,int(np.squeeze(num_detections)),1)[i], final_score[i]])
            # print (type([np.reshape(classes,int(np.squeeze(num_detections)),1)[i], final_score[i]]))
            # print ([np.reshape(classes, int(np.squeeze(num_detections)), 1)[i], final_score[i]][0])
            # print (type([np.reshape(classes, int(np.squeeze(num_detections)), 1)[i], final_score[i]][0]))
            # cv2.waitKey(0)
    ...
    # cv2.imshow('object detection', cv2.resize(image_np, (800,600)))
      ...
      if ((frame_num >= gt_st_frame) and (frame_num <= gt_ed_frame)):
          ...
          #
          #       cv2.drawMarker(image, (int(output_bb[2]), int(output_bb[1])), color, 3, 20, 1, 8)
          #       cv2.drawMarker(image, (int(output_bb[3]), int(output_bb[1])), color, 3, 20, 1, 8)
          #       cv2.drawMarker(image, (int(output_bb[2]), int(output_bb[0])), color, 3, 20, 1, 8)
          #       cv2.drawMarker(image, (int(output_bb[3]), int(output_bb[0])), color, 3, 20, 1, 8)
          #       cv2.imwrite("/home/huooi/HL_Results/MOT_result/frame_sherbrooke/out%d.jpg" % frame_num,
          #               cv2.resize(image, (int(width), int(height))))  # saving frames
          #
      ...
      # #     print ("testing the water")
      # #     # print (len(tracker.tracks[i].trace))
      # #     clr = tracker.tracks[i].track_id % 9
      # #     color = track_colors[clr]
      #     # cv2.drawMarker(image_np, (tracker.tracks[i].detection[0], tracker.tracks[i].detection[1]), color, 3, 20, 1, 8)
      #     # cv2.drawMarker(image_np, (tracker.tracks[i].prediction[0], tracker.tracks[i].prediction[1]), color, 0, 20, 1,
      #     #                8)
      #
      ...
      #
      #           # clr = tracker.tracks[i].track_id % 9
      #           # cv2.line(image_np, (int(x1), int(y1)), (int(x2), int(y2)), track_colors[clr], 2)
      #           # cv2.line(image_np, (int(x1), int(y1)), (int(x2), int(y2)), track_colors[clr], 2)
      #           # print ((tracker.tracks[i].trace[j]).shape)
      #           # cv2.drawMarker(image_np, (tracker.tracks[i].trace[j][0], tracker.tracks[i].trace[j][1]), track_colors[clr], 3, 20, 1, 8)
      #
      #           # color = track_colors[clr]
      #           # cv2.drawMarker(image_np, (tracker.tracks[i].detection[0], tracker.tracks[i].detection[1]), color, 3, 20, 1, 8)
      #           # cv2.drawMarker(image_np,(tracker.tracks[i].prediction[0], tracker.tracks[i].prediction[1]),color, 0, 20,1,8)
      #
      #

      # # Quantitative Evaluation with ground truth (dont remember why, seems attempt to manually compare)
      # if ((frame_num >= min_frame_count) and (frame_num <= max_frame_count)):
      #   cursor.execute("SELECT * FROM bounding_boxes WHERE frame_number=?", (frame_num,))
      ...
      #       obj_id = rows[i][0]
      #       # print (obj_id)
      #       cursor3.execute("SELECT * FROM bounding_boxes WHERE (frame_number = ? AND object_id = ?)",(frame_num, obj_id,))
      #       row2 = cursor3.fetchall()
      #
      #       ls_ori = [int(row2[0][3]), int(row2[0][5]), int(row2[0][2]), int(row2[0][4])] # list is in [ymin, ymax, xmin, xmax]

      # cv2.waitKey(10)
      # cv2.imshow('object detection', cv2.resize(image_np, (int(width), int(height))))
      # cv2.imwrite("/home/huooi/HL_Dataset/UrbanTracker/dummy_folder/may9/frame%d.jpg" % frame_num, cv2.resize(image_np, (int(width), int(height)))) #saving frames
      # cv2.imwrite("/home/huooi/HL_Results/MOT_result/frame_sherbrooke/out%d.jpg" % frame_num,
      #           cv2.resize(image, (int(width), int(height))))  # saving frames

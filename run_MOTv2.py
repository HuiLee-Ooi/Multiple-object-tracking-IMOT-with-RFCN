...
def main():
    ...
    # input_mode = "rene"  # "sherbrooke","rouen","rene","stmarc"
    ope_mode = "w_label"  # "w_label", "wo_label"
    # sz_imot_percent = 0.0000001
    sz_imot_percent = 0.001
    nms_thres = 0.3
    track_length_min = 6  # min length for Final track
    # track_length_min = 25
    ...
    with detection_graph.as_default():
        with tf.Session(graph=detection_graph) as sess:
            while (True):
                ...
                ls_imot = []
                ls_detection = []
                imot_src = '/home/huooi/HL_Dataset/UrbanTracker_output_imot/bgs/' + input_mode + '/' + str(
                    frame_num).zfill(8) + '.png'
                ...
                for i in range(len(ls_imot)):
                    ...
                    nested_hist = nested_hist.reshape(256 * 3, 1)
                    nested_hist = nested_hist.astype('float32')
                    imot_pack.append([[ymin, ymax, xmin, xmax], center_pt, nested_hist,
                                      [0, 0.50]])  # for comparison with detection later

                high_detection_pack = get_detection(num_detections, scores, high_detection_thres, classes, boxes, width,
                                                    height, image_np)

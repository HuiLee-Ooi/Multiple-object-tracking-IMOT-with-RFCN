...
input_dataset = "ua_detrac" # "ua_detrac", "urban_tracker"
input_mode = "test" #"stmarc","rouen","rene","stmarc" /"train", "test"
detection_thres = 0.1


def run_detector(input_file, output_file, detection_thres, visual):
            ...
            while True:
                ...
                for i in range((np.squeeze(num_detections))):
                    if (scores is None or final_score[i] > detection_thres) and (
                        ...
                        outF.write(str(frame_num) + "," + str(ymin) + "," + str(ymax) + "," + str(xmin) + "," + str(
                            xmax) + "," + str(np.reshape(classes, int(np.squeeze(num_detections)), 1)[i]) + "," + str(
                            final_score[i]))
                        outF.write("\n")
...
if (input_dataset == "urban_tracker"):
    if (input_mode == "sherbrooke"):
        input_file = '/home/huooi/HL_Dataset/UrbanTracker/sherbrooke_frames/%08d.jpg'
        output_file = output_folder + 'UrbanTracker/sherbrooke.txt'
    elif (input_mode == "rouen"):
        input_file = '/home/huooi/HL_Dataset/UrbanTracker/rouen_frames/%08d.jpg'
        output_file = output_folder + 'UrbanTracker/rouen.txt'
    elif (input_mode == "rene"):
        input_file = '/home/huooi/HL_Dataset/UrbanTracker/rene_frames/%08d.jpg'
        output_file = output_folder + 'UrbanTracker/rene.txt'
    elif (input_mode == "stmarc"):
        input_file = '/home/huooi/HL_Dataset/UrbanTracker/stmarc_frames/%08d.jpg'
        output_file = output_folder + 'UrbanTracker/stmarc.txt'
    elif (input_mode == "stmarc"):
        input_file = '/home/huooi/HL_Dataset/UrbanTracker/stmarc_frames/%08d.jpg'
        output_file = output_folder + 'UrbanTracker/stmarc.txt'
    else:
        print ("no file chosen")
    run_detector(input_file, output_file, detection_thres, False)

if (input_dataset == "ua_detrac"):
    if (input_mode == "train"):
        input_path = '/home/huooi/HL_Dataset/UA_Detrac_Detection/Dataset/Insight-MVT_Annotation_Train/'
        output_path = output_folder + '/UA_Detrac/Train/'
    elif (input_mode == "test"):
        input_path = '/home/huooi/HL_Dataset/UA_Detrac_Detection/Dataset/Insight-MVT_Annotation_Test/'
        output_path = output_folder + '/UA_Detrac/Test/'
    else:
        print ("no file chosen")
    root, dirs, files = os.walk(input_path).next()
    for i in range(len(dirs)):
        input_file = input_path + dirs[i] +'/img%05d.jpg'
        output_file = output_path + dirs[i]+'.txt'
        print (input_file)
        print (output_file)
        run_detector(input_file, output_file, detection_thres, False)
